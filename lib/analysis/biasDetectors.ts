...
export interface WatchlistAddEvent {
  timestamp: number;
  price: number;
...
export interface SymbolEvents {
  views: ViewEvent[];
  watchlistAdds: WatchlistAddEvent[];
}

export interface BehaviorInsight {
  id: string;
  type: 'recency_chasing' | 'compulsive_checking' | 'confirmation_seeking';
  symbol?: string;
  severity: 'low' | 'medium';
  detectedAt: number;
}
 ...
 */
export function detectRecencyChasing(
  symbol: string,
  events: SymbolEvents
 ...
 */
export function detectCompulsiveChecking(
  symbol: string,
  sessionViews: number[],
  views: ViewEvent[]
): BehaviorInsight | null {
 ...
 */
export function detectConfirmationSeeking(
  events: Record<string, SymbolEvents>
): BehaviorInsight | null {
  const allAdds: WatchlistAddEvent[] = [];
